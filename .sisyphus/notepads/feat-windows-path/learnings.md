...
- os.path.abspath returns OS-specific separators, which can cause mismatch in URDF string replacement on Windows.
- pathlib.Path(...).resolve().as_posix() is the preferred way to get forward-slash normalized paths in Python for cross-platform string matching.
### 2026-02-12
- URDF-embedded paths must use forward slashes for cross-platform compatibility (WebXR).
...
- Using `.as_posix()` on `pathlib.Path` objects ensures that paths are embedded in URDFs with forward slashes, which is the standard for URDF and cross-platform compatibility.
