...
This implies that:
- My previous "fix" for H1 (identity orientation) might have been incorrect if the H1 URDF is naturally Y-forward (or -Y forward).
- The `R_xr_to_robot` transform in `IKController` might still be mapping XR -Z (Forward) to Robot Y, or the `orientation` correction isn't working as intended.

## Objectives
1.  **Re-Verify Transform Chain**: Double-check `R_xr_to_robot` in `teleop_xr/ik/controller.py`.
2.  **Fix H1 Orientation**: If H1 faces Y with `identity`, and we want X, we need to rotate it by -90° (if Y is left of X) or +90° (if Y is right of X). Wait, standard ROS: X forward, Y left. So Y is +90 from X. To bring Y to X, we need -90° (yaw = -1.57).
...
3.  **Investigate Forward Direction**: Why is "converted forward direction" still +Y?
    - `compute_teleop_transform` does: `t_delta_robot = self.robot.orientation.inverse() @ R_xr_to_robot @ t_delta_xr`.
    - `R_xr_to_robot` maps XR -Z (Forward) to Canonical X.
    - If H1 faces Y (native), its `orientation` should reflect that (e.g., `orientation = rotation_from_canonical_to_native`).
    - If Native = Y-forward, then `orientation` should represent the rotation *from* X *to* Y (which is +90 deg).
    ...
    - Let's trace:
        ...
        - `t_delta_native` = `orientation.inverse() @ t_delta_canonical` = `Rot(-90) @ [1, 0, 0]` = `[0, -1, 0]` (Move -Y in native frame).
        ...
        - If `orientation` is `R_canonical_to_native`. Then `v_native = R_native_to_canonical.inv() @ v_canonical` -> `v_native = R_canonical_to_native.inv() @ v_canonical`. No.
        ...
        - **Hypothesis**: The definition of `orientation` in `BaseRobot` is "Rotation from Canonical to Base".
        ...
        - Express Canonical X in Base frame:
            ...
            - If Base is rotated +90 (Y is forward), then `X_canonical` (World Forward) aligns with `Y_base` (Robot Forward).
            - Wait, no.
            - World X is "North".
            ...
            - If H1 URDF X is "Front", but we see it facing Y, then the visualizer is rotating it, OR the `orientation` we set is rotating it.
            ...
            - If user sees it facing Y, then **H1 URDF itself is defined with Forward = Y**.
    ...
    - **Action**:
        ...
        - If we want to move "Canonical Forward" (World X), we need to compute that vector in Robot Frame.
        - `v_robot = R_world_to_robot @ v_world`
        - `R_world_to_robot = R_robot_to_world.inverse() = orientation.inverse()`.
        - `orientation = -90`. `inverse = +90`.
        - `v_robot = Rot(+90) @ [1, 0, 0] = [0, 1, 0]`.
        - If H1's "Forward" is its X axis, then `v_robot=[0, 1, 0]` moves it Left (Y).
        - If H1's "Forward" is its Y axis (as user implies), then `v_robot=[0, 1, 0]` moves it Forward.
        ...
        - To fix "Movement":
            ...
            - User said "current converted forward direction is still +y". This likely means `v_robot` is along Y.
...
- [ ] 2. Update `teleop_xr/ik/controller.py`:
    ...
    - If Robot moves Y when receiving X command, then **Robot URDF X points to World Y**.
    ...
    - **Contradiction**:
        ...
        - **Wait**: `orientation` in `BaseRobot` defines "Rotation from Canonical to Base".
        - If H1 URDF is "Left-facing" (X points Y), then Base IS rotated +90 relative to Canonical.
        ...
        - If `orientation = 0`:
            ...
            - If "Facing +Y" means "Front of robot points Y", then Robot X axis points Y.
        ...
        - **Solution**:
            ...
            - **Visual**:
                ...
                - If we want to align Robot with Canonical, we need to rotate it by `Inv(orientation)`.
                - Currently `robot_system.ts` applies `orientation` (rotation).
                - So it displays the robot IN ITS BASE ORIENTATION.
                - If Base is Y-facing, and we define `orientation=+90` (describing that state), visualizer applies +90?
            ...
            - **Resolution**:
                - `orientation` property describes "How the robot base is rotated relative to World X".
                ...
                - Visual:
                    ...
                    - This means rotating the *entire world* or rotating the robot *mesh* so X is forward.
    ...
    - **Wait, Simpler View**:
        ...
        - So `orientation = -90`?
        - If `orientation = -90`:
            - Visual: Rotates -90. Robot (Y-face) -> Rot(-90) -> X-face. **Visual Solved**.
...
4.  **Verify**: If I set `orientation = -1.57`:
    ...
    - `+1.57 @ X = Y`.
    - If Robot Native X=Right, Y=Forward. Sending Y moves Forward. Matches Visual X.
