...
def numpy2ros(transform, ros_type, frame_id="", stamp=None):
    ...
    if ros_type == geometry_msgs.msg.Pose:
        ...
        message.position.x = float(xyz[0])
        message.position.y = float(xyz[1])
        message.position.z = float(xyz[2])
        message.orientation.w = float(q[0])
        message.orientation.x = float(q[1])
        message.orientation.y = float(q[2])
        message.orientation.z = float(q[3])
    elif ros_type == geometry_msgs.msg.Transform:
        message = geometry_msgs.msg.Transform()
        message.translation.x = float(xyz[0])
        message.translation.y = float(xyz[1])
        message.translation.z = float(xyz[2])
        message.rotation.w = float(q[0])
        message.rotation.x = float(q[1])
        message.rotation.y = float(q[2])
        message.rotation.z = float(q[3])
