  ...
  async onRobotConfig(data: { urdf_url: string }) {
    ...
    try {
      const robot = await new Promise<Object3D>((resolve, reject) => {
        ...
        manager.onError = (url) => {
          console.error("[RobotModelSystem] LoadingManager error for URL:", url);
        ...
        this.loader.load(
          data.urdf_url,
          (result) => {
            robot = result as Object3D;
            console.log("[RobotModelSystem] URDF parsed, waiting for meshes...");
          },
          (progress) => {
            if (progress?.total) {
              console.log(`[RobotModelSystem] Loading progress: ${Math.round((progress.loaded / progress.total) * 100)}%`);
            }
          },
          (err) => {
            console.error("[RobotModelSystem] Loader error details:", err);
            if (err instanceof ErrorEvent) {
              console.error("[RobotModelSystem] ErrorEvent message:", err.message);
            }
            reject(err);
          }
        );
      ...
      if (error instanceof Error) {
        console.error("[RobotModelSystem] Failed to load robot:", error.message);
